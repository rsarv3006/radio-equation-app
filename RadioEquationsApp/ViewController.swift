...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "Radio Equations"
        view.addSubview(tableView)
        tableView.fillSuperview()

        let barButtonItemImage = UIImage(systemName: "gear.circle")?.withTintColor(.Theme.altColor, renderingMode: .alwaysOriginal)

        navigationItem.rightBarButtonItem = UIBarButtonItem(image: barButtonItemImage, style: .plain, target: self, action: #selector(onSettingsTapped))
        ...
        store.$hasPurchasedUnlockAlternatingCurrentEquations.sink { [weak self] hasPurchased in
            if hasPurchased {
                self?.tableView.reloadData()
            }
        }.store(in: &subscriptions)
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.viewModel = HomeScreenEquationCellViewModel(
            equation: EquationsTableInfo[indexPath.section].equations[indexPath.row]
        )
        return cell
    }
    ...
    func tableView(_: UITableView, willSelectRowAt indexPath: IndexPath) -> IndexPath? {
        ...
        if equationData.filters.contains(.alternatingCurrentFunctions) && !store.hasPurchasedUnlockAlternatingCurrentEquations {
            showMessage(withTitle: "Equation Locked", message: "Please visit the Settings Screen to unlock Alternating Current Functions.")
            return nil
        }
        ...
        navigationController?.pushViewController(equationScreen, animated: true)
